...
### 1. 🎙️ YouTube AI 오버뷰 스튜디오 (NotebookLM-Style Audio Overview)
- **다중 YouTube 소스 자동 분석**: 1개 이상의 YouTube URL을 입력받아 자동 메타데이터, 썸네일, 한/영 자막 스크립트 추출
- **Google Gemini AI 심층 교차 분석**: 여러 동영상의 핵심 아이디어, 상호 연결점, 차이점을 분석하여 남녀 2인 진행자(Host A: 민수, Host B: 지우) 팟캐스트 대화 스크립트 작성
- **초고음질 멀티스피커 TTS (Edge-TTS)**: 자연스러운 억양과 호흡을 가진 한국어 AI 음성(`ko-KR-InJoonNeural`, `ko-KR-SunHiNeural`) 합성 및 턴별 자연스러운 휴지기(Pause) 믹싱
- **인터랙티브 대본 & 플레이어**: 턴별 클릭 시 해당 타임스탬프로 즉시 점프하는 오디오 플레이어 & 말풍선 대화 UI
- **원클릭 스튜디오 브릿지**:
  - `🎬 Remotion 멀티트랙 스튜디오로 열기`: 생성된 팟캐스트 오디오, 유튜브 썸네일 비주얼, 턴별 자막 트랙을 즉시 타임라인에 로드하여 1080p MP4로 렌더링
